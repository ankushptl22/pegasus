...
import * as AllComponent from '../../json/NavigationMapping'
import "./MasterAccordion.less";
...
class MasterAccordion extends Component {
  ...
  createHeader = (header, accordians) => {
    ...
    for (var key in header) { 
      ...
      headerColumns1.push(<Grid.Column key={key}>{header[key]}</Grid.Column>);
    }
  ...
  createAccordionPanels(headerColumns, accordians, columnSequence) {
    ...
    title.data.forEach((singleData, index) => {
      ...
      columnSequence.forEach((value, key) => {
        if (title.type[value] === "string" || title.type[value] === "number") {
          singleGrids.push(
            <Grid.Column key={key} >{singleData[value]}</Grid.Column>
          );
        } else {
                var ChildComp = AllComponent[''+title.type[value]]        
                singleGrids.push(
                <Grid.Column key={key} className="components">
    ...
    content.data.forEach((singleData, index) => {
      ...
      for (var key in singleData) {
        ...
        {
          singleGrids.push(<Grid.Column key={key}>{singleData[key]}</Grid.Column>);
        } else {        
   ...
   */
  render() {
    const { headerColumns, titleGridCollection, contentGridCollection, activeIndex } = this.state;    
    ...
    for(let i = 0; i<size; i++){
        content.push(<React.Fragment key={i}>
