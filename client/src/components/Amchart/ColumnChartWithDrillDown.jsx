...
export default class ColumnChartWithDrillDown extends React.Component {
    ...
    drillDown (event) {
        if ('undefined' == typeof this.state.periodLevel[this.state.levelCount] && 
            ...
            this.state.periodLevel.push(level + ':' +highracy +':' + event.item.dataContext.period);
            this.props.drillAPIData(this.state.periodLevel);
            this.state.levelCount++;
        }
    }
        ...
        return (
            <div>
                {this.props.resultData ? (
                    <AmCharts.React
                        style={{ width: "100%", height: "300px" }} options={
                            {
                                "type": "serial",
                                "theme": "light",
